...
<template>
	...
	>
		...
		<template #cell-is_active="{ row }">
			<StatusBadge
				:tone-class="toFeaturedCategory(row).is_active ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-700'"
			>
		...
		<template #cell-actions="{ row }">
			<TableRowActions show-edit @edit="emit('edit', toFeaturedCategory(row))" @delete="emit('delete', toFeaturedCategory(row))" />
		</template>
	</DataTable>
</template>
