	...
	>
		...
		<template #cell-is_active="{ row }">
			<span
				...
				:class="toFeaturedCategory(row).is_active ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-700'"
			>
				{{ toFeaturedCategory(row).is_active ? 'true' : 'false' }}
			</span>
		</template>
		...
		<template #cell-actions="{ row }">
			<div class="flex items-center justify-end gap-2">
				<button
					...
					aria-label="Edit"
					@click="$emit('edit', toFeaturedCategory(row))"
				>
					<EditIcon />
				</button>
				<button
					...
					aria-label="Delete"
					@click="$emit('delete', toFeaturedCategory(row))"
				>
					<TrashIcon />
				</button>
			</div>
