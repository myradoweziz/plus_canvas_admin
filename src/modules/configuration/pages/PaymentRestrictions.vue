<script setup lang="ts">
	...
	import PaymentRestrictionsTable from '../components/payment-restrictions/PaymentRestrictionsTable.vue'
	...
	const rows = ref<PaymentMethodRestrictionRow[]>([])
	...
	const load = async () => {
		...
		try {
			const [restrictions, countries] = await Promise.all([
				api.getPaymentMethodRestrictions(),
			...
			else toast.error('Не удалось загрузить ограничения оплаты')
		} finally {
			loading.value = false
		}
	}
	...
	const save = async () => {
		saving.value = true
		try {
			await api.updatePaymentMethodRestrictions({
				restrictions: rows.value.map((row) => ({
					country_id: row.country_id,
					is_restricted: row.is_restricted
				}))
			})
			toast.success('Ограничения оплаты сохранены')
			await load()
		} catch (err) {
			const msg = getFirstBackendValidationMessage(err)
			if (msg) toast.error(msg)
...
<template>
	<div class="space-y-6">
		<Banner
			title="Ограничения оплаты по странам"
			subtitle="Отметьте страны, для которых оплата ограничена, и сохраните изменения."
			:icon="CountriesIcon"
			:total="rows.length"
		>
			<template #actions>
